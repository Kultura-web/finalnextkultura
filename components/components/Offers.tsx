...
export default function Offers() {
  ...
  const goToPreviousImage = (offerIndex: number) => {
    const offerImages = offers[offerIndex]?.offer_images || [];
    if (!offerImages.length) return;
    const currentIndex = currentImageIndex[offerIndex] || 0;
    const maxIndex = offerImages.length - 1;
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
          {offers.map((offer, index) => {
            const offerImages = offer.offer_images || [];
            const imageCount = offerImages.length;
            const showMultipleImages = imageCount > 1;
            const currentImage = offerImages[currentImageIndex[index] || 0]?.image_path || '';
